...
import type { ReadingMemory } from '../lib/discovery';
...
const Home = () => {
  ...
  return (
    <div className="mx-auto max-w-[1440px] space-y-10 md:space-y-12">
      ...
      {recentSongs.length > 0 && <section><div className="mb-5 flex items-end justify-between gap-4"><div><p className="eyebrow">Pick up where you left off</p><h2 className="section-heading mt-2">Your recent shelf</h2></div><Link to="/playlists" className="text-sm font-semibold text-[var(--gold-light)]">Open library <ArrowRight className="ml-1 inline h-4 w-4" /></Link></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{recentSongs.slice(0, 3).map((song) => <Link key={song.id} to={`/songs/${song.id}`} className="recent-shelf-card" onClick={() => { const memory: ReadingMemory = { ...song, thumbnailUrl: song.thumbnailUrl, language: song.language || 'en' }; rememberSong(memory); setRecentSongs(readRecentSongs()); }}><span className="recent-shelf-index">0{recentSongs.indexOf(song) + 1}</span>{song.thumbnailUrl ? <img src={song.thumbnailUrl} alt="" /> : <span className="recent-shelf-cover"><Music2 className="h-5 w-5" /></span>}<span className="min-w-0"><strong>{song.title}</strong><small>{song.artist}</small></span><ArrowRight className="ml-auto h-4 w-4 text-[var(--text-muted)]" /></Link>)}</div></section>}
