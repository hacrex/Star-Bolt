...
import { ArrowRight, Clock3, Music2, PlayCircle, Search as SearchIcon, TrendingUp } from 'lucide-react';
    ...
    <div className="mx-auto max-w-[1440px]">
      ...
      <section className="mt-12 grid gap-5 md:grid-cols-12">
        <div className="md:col-span-8">
          <div className="mb-5 flex items-end justify-between"><div><p className="eyebrow">{query ? 'Top matches' : 'Begin anywhere'}</p><h2 className="section-heading mt-2">{query ? 'Results' : 'Songs to revisit'}</h2></div>{query && <span className="font-mono text-xs text-[var(--text-muted)]">{loading ? 'Searching…' : `${results.length} matches`}</span>}</div>
          {loading ? <div className="grid gap-3" role="status" aria-label="Searching"><div className="h-24 animate-pulse rounded-xl bg-[var(--bg-surface)]" /><div className="h-24 animate-pulse rounded-xl bg-[var(--bg-surface)]" /></div> : results.length > 0 ? <div className="grid gap-3">{results.map((song) => <SongResult key={song.id} song={song} />)}</div> : query.length > 1 ? <div className="surface-card p-10 text-center text-[var(--text-secondary)]">No results found for “{query}”. Try an artist, album, or lyric phrase.</div> : <div className="grid gap-3">{recentSearches.length > 0 ? recentSearches.slice(0, 3).map((search) => <button type="button" key={search} onClick={() => handleSearch(search)} className="surface-card surface-card-hover flex items-center justify-between p-4 text-left"><span className="flex items-center gap-3"><Clock3 className="h-4 w-4 text-[var(--gold-light)]" /><span className="text-sm text-[var(--text-primary)]">{search}</span></span><ArrowRight className="h-4 w-4 text-[var(--text-muted)]" /></button>) : <div className="surface-card p-10 text-center text-[var(--text-secondary)]">Search above to find your next line.</div>}</div>}
...
const SongResult: React.FC<{ song: Song }> = ({ song }) => <Link to={`/songs/${song.id}`} className="surface-card surface-card-hover group flex items-center justify-between gap-4 p-4"><div className="flex min-w-0 items-center gap-4">{song.thumbnail_url ? <img src={song.thumbnail_url} alt={song.title} className="h-16 w-16 shrink-0 rounded-md object-cover" /> : <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-md bg-[var(--bg-elevated)] text-[var(--gold-muted)]"><Music2 className="h-7 w-7" /></div>}<div className="min-w-0"><h3 className="truncate text-base font-semibold text-[var(--text-primary)] transition-colors group-hover:text-[var(--gold-light)]">{song.title}</h3><p className="mt-1 truncate text-sm text-[var(--text-secondary)]">{song.artist}{song.album ? ` • ${song.album}` : ''}</p><p className="mt-1 truncate font-serif text-sm italic text-[var(--text-muted)]">“A line waiting to be remembered…”</p></div></div><PlayCircle className="h-5 w-5 shrink-0 text-[var(--text-muted)] transition-colors group-hover:text-[var(--gold-light)]" /></Link>;
